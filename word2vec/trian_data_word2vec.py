...
random_search_log_reg = RandomizedSearchCV(LogisticRegression(), param_grid_log_reg, n_iter=10, cv=5, n_jobs=-1, scoring='accuracy', random_state=42)
...
print("Best cross-validation accuracy for Logistic Regression: {:.2f}".format(random_search_log_reg.best_score_))
...
joblib.dump(best_log_reg_model, 'logistic_regression_model_w2v.pkl')
...
# Perform Randomized Search for SVC
random_search_svc = RandomizedSearchCV(SVC(probability=True), param_grid_svc, n_iter=10, cv=5, n_jobs=-1, scoring='accuracy', random_state=42)
...
joblib.dump(best_svc_model, 'svc_model_w2v.pkl')
...
random_search_rf = RandomizedSearchCV(RandomForestClassifier(), param_grid_rf, n_iter=50, cv=5, n_jobs=-1, scoring='accuracy', random_state=42)
...
joblib.dump(best_rf_model, 'random_forest_model_w2v.pkl')